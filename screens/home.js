...
static navigationOptions= ({navigation}) =>({ 
	...
	constructor(props) {
		...
		this.navigationWillFocusListener = props.navigation.addListener('willFocus', () => {
			...
			fetch('https://smagr173.heliohost.us/public/fetchOrderInfo.php', {
				method:'POST',
				header:{
			  ...
			  .then(() => {
				...
				.then(() => {
					fetch('https://smagr173.heliohost.us/public/fetchFavs.php', {
						method:'POST',
						header:{
						  'Accept': 'application/json',
						  'Content-type': 'application/json'
					...
					.then(() => {
						fetch('https://smagr173.heliohost.us/public/fetchHomeCount.php', {
							method:'POST',
							header:{
							  'Accept': 'application/json',
							  'Content-type': 'application/json'
						  ...
						  .then((response) => response.json())
							.then((responseJson) => {
								this.setState({
									favCount: responseJson.countFavs,
									activeCount: responseJson.countAct,
									prevCount: responseJson.countPrev,
									totalCount: responseJson.countTotal
							});
						  })
						  .catch((error) => {
							  console.log(error);
	...
	componentDidMount() {
	  ...
	  removeFav = (favID) => {
		fetch('https://smagr173.heliohost.us/public/removeFav.php', {
			method:'POST',
			header:{
			  'Accept': 'application/json',
			  'Content-type': 'application/json'
	  ...
	  favItem = (name,price,option1,option2,extra1,extra2,extra3) => {
		fetch('https://smagr173.heliohost.us/public/addFav.php', {
			method:'POST',
			header:{
			  'Accept': 'application/json',
			  'Content-type': 'application/json'
	...
	cancelOrder = () => {
		fetch('https://smagr173.heliohost.us/public/cancelOrder.php', {
			method:'POST',
			header:{
			  'Accept': 'application/json',
			  'Content-type': 'application/json'
	...
	sendToCart(itemName,itemPrice,option1,option2,extra1,extra2,extra3) {
	  fetch('https://smagr173.heliohost.us/public/addBag.php', {
		  method:'POST',
		  header:{
			'Accept': 'application/json',
			'Content-type': 'application/json'
		...
		.then((response) => response.json())
		  ...
		  if(responseJson.succ == "Success") {
			  ...
			  navigate('Cart') // Redirect to sign in page
				...
				<View>
					<FlatList
						...
						<View>
							...
							<View style={{ alignItems: 'center', marginTop: Dimensions.get('window').height*.02}}>
							  <View style={{flexDirection: 'row'}}>
								<TouchableOpacity
									onPress={() => {this.removeFav(item.fav_id)}}>
									    <View style={{flexDirection: 'row'}}>
											<View style={{ marginTop: Dimensions.get('window').height*-.0075, marginRight: Dimensions.get('window').width*.02}}>
                                                  <Ionicons
			    	                                name={"ios-close"}
				                                    size={Dimensions.get('window').height*.05}
				                                    color={'brown'}
				                                  />
                                            </View>
								...
								<TouchableOpacity
									onPress={() => {this.sendToCart(item.name,item.price,item.option1,item.option2,item.extra1,item.extra2,item.extra3)}}>
									    <View style={{flexDirection: 'row'}}>
											<View style={{ marginTop: Dimensions.get('window').height*-.001,marginLeft: Dimensions.get('window').width*.065, marginRight: Dimensions.get('window').width*.02}}>
                                                  <Ionicons
			    	                                name={"ios-refresh"}
				                                    size={Dimensions.get('window').height*.033}
													color={'brown'}
													onPress={() => {this.sendToCart(item.name,item.price,item.option1,item.option2,item.extra1,item.extra2,item.extra3)}}
				                                  />
                                            </View>
										    <Text style={styles.favLink}>Order again</Text>
										</View>
								</TouchableOpacity>
							  </View>
							</View>
	...
	expandOrder = () => {
		fetch('https://smagr173.heliohost.us/public/fetchOrder.php', {
			method:'POST',
			header:{
			  'Accept': 'application/json',
			  'Content-type': 'application/json'
			...
			<View style={styles.container}>
				...
				<View style={{backgroundColor: 'white',width: Dimensions.get('window').width,
					...
					<View style={{ flex: 1, flexDirection: 'row', alignItems: 'center'}}>
						<Image source={require('../assets/images/icon.png')}
  	   				 	 style={styles.image1} />
						<View>
						  <View style={{ marginLeft: Dimensions.get('window').width*.065, marginTop: -(Dimensions.get('window').height*.036)}}>
					    	<Text style={styles.subtitle}>Welcome, {this.state.userName}!</Text>
						  </View>
						  <View style={{ marginLeft: Dimensions.get('window').width*.09, marginTop: Dimensions.get('window').height*.008}}>
					    	<Text style={styles.subtitle3}>Total orders: {this.state.totalCount} </Text>
						  </View>
						</View>
					</View>
				</View>
				...
				<TouchableOpacity
						...
						style={{ justifyContent: 'center', backgroundColor: '#DCDCDC', width: Dimensions.get('window').width,
				                height: Dimensions.get('window').height*.1,borderBottomWidth: 2,borderColor: 'darkgray'}}>
					<View style={{ flexDirection: 'row'}}>
					    <View style={{ alignItems: 'flex-start',marginLeft: Dimensions.get('window').width*.035,marginTop: Dimensions.get('window').height *.009}}>
						    <Text style={styles.subtitle}>Favorites ({this.state.favCount})</Text>
					    </View>
					    <View style={{flex: 1, marginRight: Dimensions.get('window').width *.065,alignItems: 'flex-end', marginTop: Dimensions.get('window').height *.009}}>
						    {(this.state.visible3 == true) ?  <Ionicons
				                name={'ios-arrow-up'}
				...
				<TouchableOpacity
					...
					<View style={{ flexDirection: 'row'}}>
					    <View style={{ alignItems: 'flex-start',marginLeft: Dimensions.get('window').width*.035,marginTop: Dimensions.get('window').height *.009}}>
						    <Text style={styles.subtitle}>Active Orders ({this.state.activeCount})</Text>
					    </View>
					    <View style={{flex: 1, marginRight: Dimensions.get('window').width *.065,alignItems: 'flex-end', marginTop: Dimensions.get('window').height *.009}}>
						    {(this.state.visible == true) ?  <Ionicons
				                name={'ios-arrow-up'}
				                size={Dimensions.get('window').width*.065}
				                color={'black'}
			                /> :
				...
				<TouchableOpacity
					...
					<View style={{ flexDirection: 'row'}}>
					    <View style={{ alignItems: 'flex-start',marginLeft: Dimensions.get('window').width*.035,marginTop: Dimensions.get('window').height *.009}}>
						    <Text style={styles.subtitle}>Previous Orders ({this.state.prevCount})</Text>
					    </View>
					    <View style={{flex: 1, marginRight: Dimensions.get('window').width *.065,alignItems: 'flex-end', marginTop: Dimensions.get('window').height *.009}}>
						    {(this.state.visible2 == true) ?  <Ionicons
				                name={'ios-arrow-up'}
				                size={Dimensions.get('window').width*.065}
				                color={'black'}
			                /> :
